...
if __name__ == "__main__":
    ...
    avg_values = {
        'piece_name': 'Average',
        'H1': np.mean(result_dict['H1']),
        'H4': np.mean(result_dict['H4']),
        'GS': np.mean(result_dict['GS']),
    }

    # Append averages as a summary row
    for key in avg_values.keys():
        if key != 'piece_name':
            result_dict[key].append(avg_values[key])
    result_dict['piece_name'].append(avg_values['piece_name'])

    # Save results to CSV
    if len(result_dict):
        df = pd.DataFrame.from_dict(result_dict)
        df.to_csv(opt.output_csv, index=False, encoding='utf-8')
